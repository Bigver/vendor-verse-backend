...
export const DailyIncome = async (req, res) => {
 ...
 try {
    ...
    const orders = await OrderRestaurant.findAll({
      where: {
        store_id: store_id,
...
export const OverviewWebsite = async (req, res) => {
  ...
  try {
      ...
      const pendingOrders = await OrderRestaurant.count({
          where: { payment_status: "กำลังตรวจสอบ" , store_id : store_id }, // กำหนดสถานะคำสั่งซื้อที่ต้องการ
      });
