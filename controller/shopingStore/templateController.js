...
import Template from "../../model/shopingStore/templateModel.js";
import { Op } from "sequelize";

export const createTemplate = async (req, res) => {
  const { category, template_id, img } = req.body;
   try {
    const newTemplate = await Template.create({ category, template_id, img });
...
export const getTemplateByCategoryAndId = async (req, res) => {
  const { category, template_id } = req.params; // ดึง category และ template_id จาก params
  try {
